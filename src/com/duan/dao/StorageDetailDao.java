...
public class StorageDetailDao
{
	...
	public static boolean update(StorageDetail stor, int storageId) throws SQLException
    {
        String sql = " UPDATE STORAGE_DETAIL SET storage_id=? book_id=? description=? price=? WHERE storage_id = ?";
					        		
        PreparedStatement pre = JDBCHelper.createPreparedStatement(sql,stor.getStorageId(),stor.getBookId(),
        														stor.getAmount(),stor.getPrice(), storageId);
		int count = pre.executeUpdate();
